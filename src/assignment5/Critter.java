...
public abstract class Critter {
    ...
    protected String look(int direction, boolean steps) {
		...
		switch (direction){
			case 0: // 0: (1,0) 
				if(steps && worldMap[xWrap(x_coord,1)][y_coord] != null && worldMap[xWrap(x_coord,1)][y_coord].neighbors.size() > 0  ) {
					return worldMap[xWrap(x_coord,1)][y_coord].neighbors.get(0).toString();					
				}
				else if (worldMap[xWrap(x_coord,2)][y_coord] != null && worldMap[xWrap(x_coord,2)][y_coord].neighbors.size() > 0){
					return worldMap[xWrap(x_coord,2)][y_coord].neighbors.get(0).toString();
				}
				break;
			case 1: // 1: (1,-1)
				if(steps && worldMap[xWrap(x_coord,1)][yWrap(y_coord,-1)] != null && worldMap[xWrap(x_coord,1)][yWrap(y_coord,-1)].neighbors.size() > 0 ) {
					return worldMap[xWrap(x_coord,1)][yWrap(y_coord,-1)].neighbors.get(0).toString();					
				}
				else if (!steps && worldMap[xWrap(x_coord,2)][yWrap(y_coord,-2)] != null && worldMap[xWrap(x_coord,2)][yWrap(y_coord,-2)].neighbors.size() > 0 ){
					return worldMap[xWrap(x_coord,2)][yWrap(y_coord,-2)].neighbors.get(0).toString();
				}
				break;
			case 2: // 2: (0,-1)
				if(steps && worldMap[x_coord][yWrap(y_coord,-1)] != null && worldMap[x_coord][yWrap(y_coord,-1)].neighbors.size() > 0) {
					return worldMap[x_coord][yWrap(y_coord,-1)].neighbors.get(0).toString();					
				}
				else if (!steps && worldMap[x_coord][yWrap(y_coord,-2)] != null && worldMap[x_coord][yWrap(y_coord,-2)].neighbors.size() > 0){
			...
			case 4: // 4: (-1,0)
				...
				else if (!steps && worldMap[xWrap(x_coord,-2)][y_coord] != null && worldMap[xWrap(x_coord,-2)][y_coord].neighbors.size()>0){
					return worldMap[xWrap(x_coord,-2)][y_coord].neighbors.get(0).toString();
				}
				break;
			case 5: // 5: (-1,1)
